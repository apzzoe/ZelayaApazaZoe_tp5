...
pnodo buscar_por_indice(tlista lista, int indice){
	...
	cout<<"\nLista luego de eliminar último nodo:\n";
	mostrar(lista);
	cout<<"\nCantidad de elementos: " << obtener_cantidad(lista)<<endl;
	...
	if(indice != -1){
		cout<<"El valor "<<valor_buscar<<" se encuentra en la posición lógica: "<<indice<<endl;
	}
	else{
	...
	int posicion;
	cout<<"\nIngrese una posición para buscar el nodo: ";
	cin>>posicion;

	pnodo encontrado = buscar_por_indice(lista, posicion);
	if(encontrado != NULL){
		cout<<"En la posición "<<posicion<<" se encuentra el valor: "<<encontrado->dato<<endl;
	}
	else{
	    cout<<"No hay ningún nodo en esa posición.\n";
	}
    }
